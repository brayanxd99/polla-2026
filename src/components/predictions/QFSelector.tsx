...
interface QFSelectorProps {
  teams: Team[]
  initialSelectedIds: string[]
}

export function QFSelector({ teams, initialSelectedIds }: QFSelectorProps) {
  ...
  const toggleTeam = (teamId: string) => {
    ...
    if (selectedIds.includes(teamId)) {
      ...
      if (selectedIds.length >= 8) {
        setError("Solo puedes seleccionar un máximo de 8 equipos.")
  ...
  const handleSave = async () => {
    if (selectedIds.length !== 8) {
      setError("Debes seleccionar exactamente 8 equipos para guardar.")
      setTimeout(() => setError(null), 3000)
      return
    }
    ...
    try {
      const res = await fetch("/api/predictions/qf", {
        method: "POST",
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
        <div>
          <h2 className="text-xl font-bold text-polla-neon flex items-center gap-2">
            ...
            Predicción Cuartos de Final
          </h2>
          <p className="text-sm text-muted-foreground mt-1">Selecciona los 8 equipos que clasificarán. (+5 puntos por acierto)</p>
        </div>
        <div className="flex items-center gap-4">
          <div className="bg-white/5 px-4 py-2 rounded-xl border border-white/10 font-bold">
            <span className={selectedIds.length === 8 ? "text-green-400" : "text-white"}>{selectedIds.length}</span> / 8
          </div>
          <Button 
            onClick={handleSave} 
            disabled={isSaving || selectedIds.length !== 8 || isLocked}
